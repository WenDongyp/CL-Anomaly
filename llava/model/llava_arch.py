...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        #(3,384,384)
        #for idex, item in enumerate(images):
        #    rank_print(idex,"images shape", item.shape)
        #rank_print("*******************************")
        #(729,1152)
        image_features = self.get_model().get_vision_tower()(images)

        global_image_features = image_features.mean(dim=1)

        image_features = self.get_model().mm_projector(image_features)
        #return clip_image_features.to(self.device), image_features.to(self.device)
        return global_image_features, image_features
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
        ...
        #print("*********")
        #for item in images:
        #    rank_print("images shaep", item.shape)
        #rank_print("*******************************")
        ...
        if type(images) is list or images.ndim == 5:
            ...
            global_image_features, encoded_image_features = self.encode_images(concat_images)
            ...
            global_image_features, image_features = self.encode_images(images)

        image_guide_features = global_image_features
        #for idex, item in enumerate(image_guide_features):
        #    rank_print(idex, "global_features", item.shape)
        #rank_print("*******************************************")
        #print("image feature shape", image_features.shape)
        if self.training:

            current_image_features = image_guide_features  # [batch_size, feature_dim]
            #rank_print("self cur task", self.cur_task)
            task_id = self.cur_task
            #rank_print("image anchors", (self.image_anchors[task_id]).shape)
            #rank_print("image boundary", (self.image_boundary[task_id]).shape)
            #print("current image features", (current_image_features.sum(dim=0).shape))
            #rank_print("current_image_features shapes:", [tuple(f.shape) for f in current_image_features])
            ...
            for layer_idx in range(len(self.model.layers)):
                ...
                for proj_name in proj_names:
                    ...
                    # write the same weights to the current layer
