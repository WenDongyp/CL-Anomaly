...
from CLADMoE.peft import PeftModel, TaskType, get_peft_model, CLADMoELoraConfig, WEIGHTS_NAME, set_peft_model_state_dict
...
class ModelArguments:
    ...
    private_expert_num: Optional[int] = field(default=4)
...
def find_all_linear_names(model):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, cls):
            ...
            lora_module_names.add(name)   # keep full path to avoid duplicate names

    if "lm_head" in lora_module_names:  # needed for 16-bit
        lora_module_names.remove("lm_head")
    # print all layers that will get LoRA
    print("LoRA layers:\n", sorted(lora_module_names))
    return list(lora_module_names)


def find_all_linear_names_vision(model):
    cls = torch.nn.Linear
    lora_module_names = set()
    ...
    if "lm_head" in lora_module_names:  # needed for 16-bit
        lora_module_names.remove("lm_head")
    # print all layers that will get LoRA
    print("LoRA layers in vision_tower:\n", sorted(lora_module_names))

    return list(lora_module_names)

def find_all_linear_names_any(model, verbose=False):
    """
    Return full path names of *all* nn.Linear in the model, excluding lm_head.
    When verbose=True, each retained layer name will be printed.
    """
    linear_names = []
    for name, module in model.named_modules():
...
def load_model_from_previous_task(model, previous_task_model_path):
    ...
    before = {k: v.clone() for k, v in model.base_model.model.named_parameters() if 'image_anchors' in k}
    model.base_model.model.load_state_dict(non_lora_trainables, strict=False)

    for k, v_after in model.base_model.model.named_parameters():
        if 'image_anchors' in k:
            changed = not torch.equal(before[k].cpu(), v_after.cpu())
            print(f"{k}: {'CHANGED' if changed else 'UNCHANGED'}")
    ...
    print('Loading CL-Anomaly weights from ', filename)
    ...
    #for k, v_after in model.named_parameters():
    #    if k in before:
    #        changed = not torch.equal(before[k].cpu(), v_after.cpu())
    #        print(f"{k}: {'CHANGED' if changed else 'UNCHANGED'}")
...
def train(attn_implementation=None):
    ...
    with open("/data1/dongwen_data/CL-Anomaly/router/cur_task.txt", "w") as t:
        t.write(model_args.task)
    print("cur task", model_args.task)
    ...
    if training_args.lora_enable:
        kwargs = {
                ...
                "private_expert_num": model_args.private_expert_num,
                "cur_task": model_args.cur_task,
            }
        lora_config = CLADMoELoraConfig(
            r=training_args.lora_r,
            lora_alpha=training_args.lora_alpha,
            target_modules=find_all_linear_names(model),
            lora_dropout=training_args.lora_dropout,
            bias=training_args.lora_bias,
            task_type=TaskType.CAUSAL_LM_CLADMoE,
            **kwargs
        )
    ...
    model.set_cur_task(model_args.cur_task, model_args.expert_num, model_args.private_expert_num)
