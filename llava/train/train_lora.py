...
from CLMoE.peft import SAFETENSORS_WEIGHTS_NAME, set_peft_model_state_dict
...
def load_model_from_previous_task(model, previous_task_model_path):
    ...
    filename = os.path.join(previous_task_model_path, SAFETENSORS_WEIGHTS_NAME)
    adapters_weights = load_file(filename, device="cpu")
    adapter_device = next(model.parameters()).device  # or torch.cuda.current_device()
    adapters_weights = {k: v.to(adapter_device) for k, v in adapters_weights.items()}
